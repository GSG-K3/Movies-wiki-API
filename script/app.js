...
const buildMoves = response => {
  for (let i = 0; i < 4; i++) {
    ...
    infoButton.onclick = function() {
      ...
      if (i == 0) {
        apiCall(searchUrl, wikifunction);
      }
      if (i == 1) {
        apiCall(searchUrl1, wikifunction);
      }
      if (i == 2) {
        apiCall(searchUrl2, wikifunction);
      }
      if (i == 3) {
        apiCall(searchUrl3, wikifunction);
      }

      let _model = document.getElementById("myModel");
      if (model !== undefined) _model.style.display = "block";
    };
  }
};
